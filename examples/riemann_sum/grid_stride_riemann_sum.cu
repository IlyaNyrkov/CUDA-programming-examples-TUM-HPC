...
__global__ void gridStrideRiemannSum(T a, T dx, int N, T *partial_sums) {
    ...
    if (tid < 32) {
        warpReduceUnrolled<blockSize, T>(subArray, tid);
    }

    if (tid == 0) {
        partial_sums[blockIdx.x] = subArray[0];
    }
}
...
int main(int argc, char* argv[]) {
    ...
    const int BLOCKS = (N + THREADS * 2 - 1) / (THREADS * 2);  // Fixed parentheses
    double dx = (b - a) / N;
    ...
    gridStrideRiemannSum<double, complex_kernel_gpu, BLOCK_SIZE>
        <<<BLOCKS, THREADS, sizeof(double) * THREADS>>>(a, dx, N, d_partial_sums);
